...
const res = await client.fetchAll(q("SELECT p.id, title, published, likes, created_at, u.username FROM post p LEFT JOIN account u ON u.id = p.account_id WHERE p.id = $1"), 1);
console.log({ res })
