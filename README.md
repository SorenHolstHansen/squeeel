...
Try going into the examples/postgres/basic, start a postgres database, and run the migration in the migration file. After that, run `bun build index.ts --target bun` (You need to export env vars as specified [here](https://node-postgres.com/features/connecting#environment-variables)), then (possibly after restarting your lsp), you should now have the right types for the query results.
